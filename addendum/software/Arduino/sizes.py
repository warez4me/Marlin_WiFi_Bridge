...
def main():
    ...
    # ---  bin copy for OTA starts here ---
    try:
        import shutil
        build_dir = os.path.dirname(args.elf)
        
        for file in os.listdir(build_dir):
            if file.endswith('.bin.signed') or (file.endswith('.bin') and not file.endswith('.elf.bin')):
                src_file = os.path.join(build_dir, file)
                dst_file = r"c:\Users\Public\Documents\Arduino\OTA_BIN\file4ota.bin"
                
                shutil.copy(src_file, dst_file)
                print(f"\n[AUTO-OTA] .bin saved to OTA_BIN!")
                break
    except Exception as e:
        print(f"\n[AUTO-OTA] bin-copy error: {e}")
    # ---  bin copy for OTA ends here ---
