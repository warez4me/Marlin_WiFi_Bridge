...
1. **Enable Package Inclusions:** Ensure your root `configuration.yaml` allows named directory package inclusions under the primary `homeassistant:` domain:
   ...
   ```
2. **Deploy the Asset:** Copy the pre-configured package file `iot_7.yaml` into your server's packages directory, using your desired filename instead of the default `iot_7`:
...
The provided layout file (`dashboard/full_view_ui.yaml`) already includes a dedicated embedded panel section. Within that configuration text, you will find an explicit URL path property mapping directly to the bridge server resource:
